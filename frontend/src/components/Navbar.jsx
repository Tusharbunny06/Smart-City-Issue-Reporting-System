...
import { LogOut, Building2, User, ShieldCheck, Bell, HardHat } from 'lucide-react';
...
const Navbar = () => {
  ...
  const getShiftPill = () => {
    ...
    if (isOnShift) {
      return (
        <div className="flex items-center space-x-1.5 bg-bg-tertiary border border-theme-border px-3 py-1.5 rounded-full text-sm font-medium">
          <div className="w-2 h-2 bg-success rounded-full"></div>
          <span className="text-text-primary">On Shift <span className="text-text-muted text-xs">· {String(shiftStart).padStart(2, '0')}:00–{String(shiftEnd).padStart(2, '0')}:00</span></span>
        </div>
      );
    } else {
      return (
        <div className="flex items-center space-x-1.5 bg-bg-tertiary border border-theme-border px-3 py-1.5 rounded-full text-sm font-medium">
          ...
          <span className="text-text-secondary">Off Duty <span className="text-text-muted text-xs">· Next shift: {String(shiftStart).padStart(2, '0')}:00</span></span>
        </div>
      );
    }
  };
            ...
            <>
              {user.role === 'Worker' && (
                <div className="absolute left-1/2 transform -translate-x-1/2 hidden md:flex items-center space-x-2">
                  ...
                  <span className="bg-[rgba(245,158,11,0.15)] text-accent text-xs font-bold px-2 py-0.5 rounded-full">
                    {user.role}
                  </span>
                </div>
              )}
